...
# 2. 智能密钥探测引擎 (Secrets First 逻辑)
api_key = None

# 优先读取云端加密暗房
if "FRED_API_KEY" in st.secrets:
    api_key = st.secrets["FRED_API_KEY"].strip()
    st.sidebar.success("🔒 已从云端安全暗房成功加载加密 API Key")
else:
    # 备用方案：如果后台没配，才显示左侧输入框
    st.sidebar.header("⚙️ 核心配置")
    api_key_input = st.sidebar.text_input("输入你的 FRED API Key", type="password")
    if api_key_input:
        api_key = api_key_input.strip()
    st.sidebar.markdown("""
    [如何获取免费 API Key?](https://fredapi.stlouisfed.org/login/apikeys)
    *提示：为了免输入，建议在 Streamlit 后台配置 Secrets。*
    """)

if not api_key:
    st.info("💡 请在 Streamlit 平台后台配置 Secrets，或在左侧边栏输入 Key 以激活数据流。")
    st.stop()
...
try:
    ...
    st.error(f"API Key 验证失败，请检查输入或后台 Secrets 配置。错误信息: {e}")
    st.stop()
...
def fetch_and_process_wicksellian_data():
    ...
    df_gdp['gdp_yoy'] = df_gdp['gdp_raw'].pct_change(periods=4) * 100
    df_cpi['cpi_yoy'] = df_cpi['cpi_raw'].pct_change(periods=12) * 100
